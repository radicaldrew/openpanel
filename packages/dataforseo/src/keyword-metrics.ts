...
// `intent` is the raw `main_intent` (null for Google Ads). `competition` is a
// 0-1 ratio.
...
function normalizeKeywordOverview(
  ...
  return {
    ...
    monthlySearches: toMonthlySearches(
      usesClickstream ? clickstreamInfo?.monthly_searches : info?.monthly_searches,
    ),
  };
}
...
function normalizeAdsKeyword(item: AdsKeywordItem, keyword: string): KeywordMetricRow {
  return {
    keyword,
...
export async function fetchKeywordMetricsForList(
  ...
  for (let i = 0; i < params.keywords.length; i += KEYWORD_METRICS_BATCH_SIZE) {
    ...
    if (useGoogleAds) {
      const { data: items } = await client.keywords.adsSearchVolume({
        keywords,
      ...
      const [ads, labs] = await Promise.all([
      ...
      rows.push(...mergeLocalAndNationalRows(keywords, ads.data, labs.data));
    } else {
      const { data: items } = await client.labs.keywordOverview({
        keywords,
        locationCode: params.locationCode,
        languageCode: params.languageCode,
        includeClickstreamData: params.includeClickstreamData ?? false,
...
function mergeLocalAndNationalRows(
  ...
  const labsByKeyword = new Map<string, KeywordOverviewItem>();
  for (const item of labsItems) {
    if (item.keyword) {
      labsByKeyword.set(item.keyword.toLowerCase(), item);
    }
  }
  const rows: KeywordMetricRow[] = [];
  const covered = new Set<string>();
  ...
  for (const keyword of keywords) {
    if (covered.has(keyword.toLowerCase())) {
      continue;
    }
