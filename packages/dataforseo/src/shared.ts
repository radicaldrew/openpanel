// Constants and target builders shared between consumers and the section
// fetchers. Keep this module free of section-file imports so both sides can
...
export function buildLlmTarget(input: {
  ...
  value: string;
  /**
   * Domain targets only. Defaults to subdomains included; research scopes
   * narrower than `subdomains` pass `false`. There is no URL/path-level
   * targeting in this API — page-level scoping happens by post-filtering the
   * returned page URLs.
   */
