// Supported keyword-data countries, SERP languages, and their data provider.
//
...
// full Google geotarget list. Google-Ads-only rows have no keyword difficulty
// or search intent.
//
...
export function getIsoCountryCode(locationCode: number): string {
  const shortLabel =
    LOCATION_OPTIONS.find((option) => option.code === locationCode)?.shortLabel ?? 'US';
 ...
 */
export const SERP_LANGUAGE_OPTIONS: readonly LanguageOption[] = [
