...
export function Chart({ data, annotations }: Props) {
  ...
  // Over every series, not the visible subset, so toggling one off does not
  // recolour the rest.
  ...
  const getMenuItems = useCallback(
    (e: any, clickedData: any): ChartClickMenuItem[] => {
      ...
      // Which line was under the cursor. `activePayload` carries one entry per
      // series drawn at that x, and the data keys are `${serieId}:count`.
              ...
              {series.map((serie) => {
                ...
                return (
                  <Area
                    ...
                    // Stacked PER AXIS. Adding a latency on the right axis to a
                    // request rate on the left produces a number that measures
                    // nothing; an events report puts everything on `left` and
                    // so stacks exactly as it always has.
                    stackId={serieAxis(serie)}
                    type={lineType}
                    name={serie.id}
