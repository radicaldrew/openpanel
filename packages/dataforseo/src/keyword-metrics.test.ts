...
import { fetchKeywordMetricsForList, type KeywordMetricsClient } from './keyword-metrics';

const billing = { path: ['v3'], costUsd: 0 };

// Minimal fake client exposing only the two endpoints the helper touches.
function fakeClient(overrides: {
  keywordOverview?: KeywordMetricsClient['labs']['keywordOverview'];
  adsSearchVolume?: KeywordMetricsClient['keywords']['adsSearchVolume'];
...
describe('fetchKeywordMetricsForList', () => {
  ...
  it('batches keyword lists above the per-call cap', async () => {
    const keywordOverview = vi.fn().mockImplementation(({ keywords }: { keywords: string[] }) =>
      Promise.resolve({
        billing,
        data: keywords.map((keyword) => ({ keyword, keyword_info: {} })),
      }),
    );
    const client = fakeClient({ keywordOverview });
  ...
  it('merges city-scoped Ads volume with national Labs KD for local requests', async () => {
    ...
    expect(adsSearchVolume).toHaveBeenCalledWith(
      expect.objectContaining({ locationName: 'Springfield,Illinois,United States' }),
    );
