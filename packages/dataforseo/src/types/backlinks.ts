 ...
 */
export const backlinksScopeWithLegacySchema = z.enum([...RESEARCH_SCOPES, 'page']);

export type BacklinksScopeWithLegacy = z.infer<typeof backlinksScopeWithLegacySchema>;

export function resolveBacklinksScope(scope: BacklinksScopeWithLegacy): ResearchScope {
...
/** Shared wording for MCP tools that take a backlinks scope. */
...
export interface BacklinksSpamFilterOptions {
  hideSpam?: boolean;
  spamThreshold?: number;
}

export function normalizeBacklinksSpamFilterOptions(options?: BacklinksSpamFilterOptions) {
  const hideSpam = options?.hideSpam ?? true;
  return {
    hideSpam,
    spamThreshold: hideSpam
      ? normalizeBacklinksSpamThreshold(options?.spamThreshold ?? DEFAULT_BACKLINKS_SPAM_THRESHOLD)
      : undefined,
  };
}
...
export const backlinksRowsSortFieldSchema = z.enum(['rank', 'domainRank', 'spamScore', 'firstSeen']);
...
/** Single source for each tab's default sort. */
...
const backlinksPageRequestBase = backlinksLookupSchema.extend({
  ...
  pageSize: z
    .number()
    .int()
    .refine((value) => (BACKLINKS_PAGE_SIZES as readonly number[]).includes(value))
...
export const backlinksRowsPageRequestSchema = backlinksPageRequestBase.extend({
  sortField: backlinksRowsSortFieldSchema.default(BACKLINKS_DEFAULT_SORT.backlinks.field),
  filters: backlinksRowsFiltersSchema.default({}),
  mode: backlinksRowsModeSchema.default('one_per_domain'),
});

export const referringDomainsPageRequestSchema = backlinksPageRequestBase.extend({
  sortField: referringDomainsSortFieldSchema.default(BACKLINKS_DEFAULT_SORT.domains.field),
  filters: referringDomainsFiltersSchema.default({}),
});

export const topPagesPageRequestSchema = backlinksPageRequestBase.extend({
  sortField: topPagesSortFieldSchema.default(BACKLINKS_DEFAULT_SORT.pages.field),
...
export type BacklinksRowsSortField = z.infer<typeof backlinksRowsSortFieldSchema>;
export type ReferringDomainsSortField = z.infer<typeof referringDomainsSortFieldSchema>;
...
export type ReferringDomainsFilters = z.infer<typeof referringDomainsFiltersSchema>;
export type TopPagesFilters = z.infer<typeof topPagesFiltersSchema>;
export type BacklinksRowsPageInput = z.infer<typeof backlinksRowsPageRequestSchema>;
export type ReferringDomainsPageInput = z.infer<typeof referringDomainsPageRequestSchema>;
